        ...
        if (interaction.customId == "select") {
            ...
            if (interaction.values[0] == "yetkili") {
                interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
                    ...
                    .setDescription("Merhaba, ben **FİGHT FOR GLORY CONTACT**. size nasıl rol alıcağınız hakkında bilgi vereceğim. Diamond rolüne sahip olmak istiyorsanız sayısı fark etmezsizin result atmanız yeterlidir yetkililer gelip bakacaktır. Pmco rolüne sahip olmak istiyorsanız **luqipedia** linki ve logo atmanız yeterlidir yetkililer gelip bakacaktır.")
...
> **Seni ve takımını sunucumuzda görmekten çok memnun oluruz aşağıdaki linke dokunarak bize katılabilirsiniz**
 ...
 else if (interaction.values[0] == "basvur") {
                interaction.guild.channels.create(`ticket-${interaction.user.username}`, {
                    type: 'GUILD_TEXT',
                    topic: `${interaction.user.id}`,
                    parent: `${catégorie}`,
                    ...
                    const embed = new MessageEmbed()
                    .setTitle('Staff başvurusu için bilet')
                    .setDescription('Lütfen başvurunuzu detaylandırın, böylece bir sunucu moderatörü sorumluluğu üstlenecek.')
                    .setFooter('FİGHT FOR GLORY CONTACT')
                    c.send({embeds: [embed], content: `${roleStaff} | ${interaction.user}`, components: [row]})
                    interaction.reply({content: `Biletiniz başarıyla açıldı. <#${c.id}>`, ephemeral: true})
